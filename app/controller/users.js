...
const getUsers = require("../schema/users");
...
class UserController extends Controller {
  ...
  async register() {
    ...
    const params = JSON.stringify({ username: "Admin", password: "123456" }); // Object.keys(this.ctx.request.body)[0];
    // Array.from(new Array(300)).forEach(() => {
    //     let ss = {
    //         username: getUsers(),
    //         password: 123456
    //     };
    //     this.service.users.register(ss)
    // });
  ...
  async logout() {
    const accessToken = this.ctx.request.header["authorization"].replace(
      "Bearer ",
      ""
    );

    const result = await this.ctx.service.refreshToken.delRefreshToken(
      accessToken
    );
