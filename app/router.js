...
module.exports = app => {
  ...
  // restful api
  app.post('/api/users/authorize', app.oAuth2Server.token(), app.controller.users.authorize);
  app.post('/api/users/register', app.controller.users.register);
  app.post('/api/users/logout', app.controller.users.logout);
  app.get('/api/users', app.oAuth2Server.authenticate(), app.controller.users.index);
  app.get('/api/users/profile/:userId', app.oAuth2Server.authenticate(), app.controller.users.profile);
